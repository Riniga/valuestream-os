# RACI-matriser – Vem Gör Vad?

RACI-matriser definierar för varje process-fas vem som är ansvarig, accountable, consulted och informed.

## RACI-matriser per Fas

- **[Kravställning RACI](./1.%20Kravställning.md)** – Ansvar i requirements-fasen
- **[Målarkitektur RACI](./2.%20Målarkitektur.md)** – Ansvar i design-fasen
- **[Roadmap RACI](./3.%20Roadmap.md)** – Ansvar i planning-fasen
- **[Leverans RACI](./4.%20Leverans.md)** – Ansvar i delivery-fasen
- **[Repeat RACI](./5.%20Repeat.md)** – Ansvar i learning-fasen

**RACI-definitionen:**

- **Responsible (R)** – Vem gör jobbet?
- **Accountable (A)** – Vem fattar det slutgiltiga beslutet?
- **Consulted (C)** – Vem bör granska och ge återkoppling in innan beslutet?
- **Informed (I)** – Vem måste informeras efter beslutet?

---

📱 **[← Tillbaka till Framework INDEX](../INDEX.md)**

---

_Använd RACI-matrisen för att förstå rollen och ansvaret för varje role i varje fas._
