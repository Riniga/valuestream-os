# Agenter – Roller i ValueStream OS

Varje agent representerar en roll med definierat ansvar, perspektiv och arbetsprinciper.

## Agenter

- [Business Analyst](./business-analyst.md) – Analysera och strukturera krav
- [Solution Architect](./lösningsarkitekt.md) – Designa målarkitektur
- [Developer](./utvecklare.md) – Implementera lösningen
- [UX Designer](./ux.md) – Designa användarupplevelse
- [Product Owner](./produktägare.md) – Äga affärsvärde och prioritering
- [Project Manager](./projektledare.md) – Koordinera projekt
- [Scrum Master](./scrum-master.md) – Facilitera teamarbete
- [Business Expert](./verksamhetsexperter.md) – Subject matter expertise
- [Technical Lead](./teknisk-lead.md) – Teknisk vägledning
- [Enterprise Architect](./enterprise-arkitekt.md) – Enterprise-wide consistency
- [Data Architect](./dataarkitekt.md) – Data design och governance
- [DevOps](./devops.md) – Infrastructure och deployment
- [QA/Tester](./testare.md) – Quality assurance
- [User Representatives](./användarrepresentanter.md) – End-user perspective

---

📱 **[← Tillbaka till Framework INDEX](../INDEX.md)**

---

*Till ett initiativ kan du välja relevanta agenter baserat på projekt-scope.*
